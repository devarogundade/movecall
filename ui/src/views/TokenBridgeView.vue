...
const bridge = async () => {
    ...
    if (form.value.token.address == zeroAddress) {
        tx = await Contract.tokenTransferETH(
            parseUnits(form.value.amount.toString(), form.value.token.decimals),
            form.value.receiver,
        );
    } else {
        tx = await Contract.tokenTransfer(
            form.value.token.address,
            parseUnits(form.value.amount.toString(), form.value.token.decimals),
            form.value.receiver,
        );
                    ...
                    <div class="container">
                        ...
                        <div class="input2">
                            <div class="labels">
                                <label>Amount</label>
                                <div class="bal">
                                    <p>Bal: {{ Converter.toMoney(walletStore.balances[form.token.address]) }} {{
                                        form.token.symbol }}
                                    </p>
                                    <a href="https://cloud.google.com/application/web3/faucet/ethereum/holesky"
                                        target="_blank" v-if="form.token.address == zeroAddress">
                        ...
                        <div class="input2">
                            ...
                            <input type="text"
                                placeholder="0x264d28e6e657b8d099cefafc7d186f15f74b88bc76882feb31bcd81d7804ddd9"
